...
public class UIAtlasMaker : EditorWindow
{
	...
	static public bool UpdateTexture (INGUIAtlas atlas, List<SpriteEntry> sprites)
	{
        ...
        if (false == newTexture)
        {
			...
			tiAtlas.isReadable = true;
            var settings = tiAtlas.GetPlatformTextureSettings("Android");
            settings.overridden = false;
            tiAtlas.SetPlatformTextureSettings(settings);

            settings = tiAtlas.GetPlatformTextureSettings("iPhone");
            settings.overridden = false;
            tiAtlas.SetPlatformTextureSettings(settings);
